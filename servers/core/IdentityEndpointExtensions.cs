...
public static class IdentityEndpointExtensions
{
    ...
    /// </summary>
    /// <typeparam name="TUser">The type describing the user. This should match the generic parameter in <see
    /// cref="UserManager{TUser}"/>.</typeparam> <param name="endpoints"> The <see cref="IEndpointRouteBuilder"/> to add
    /// the identity endpoints to. Call <see cref="EndpointRouteBuilderExtensions.MapGroup(IEndpointRouteBuilder,
        ...
        where TUser : class, new()
    {
        ...
        // NOTE: We cannot inject UserManager<TUser> directly because the TUser generic parameter is currently
        // unsupported by RDG. https://github.com/dotnet/aspnetcore/issues/47338
        // routeGroup.MapPost("/register",
        //                    async Task<Results<Ok, ValidationProblem>> ([FromBody] RegisterRequest registration,
        //                                                               HttpContext context,
        ...
        routeGroup
            .MapGet("/confirmEmail",
                        ...
                        [FromServices] IServiceProvider sp) =>
                    {
                        ...
                        {
                            // As with Identity UI, email and user name are one and the same. So when we update the
                            // email, we need to update the user name.
        ...
        routeGroup.MapPost("/resendConfirmationEmail",
                           async Task<Ok> ([FromBody] ResendConfirmationEmailRequest resendRequest, HttpContext context,
                                          [FromServices] IServiceProvider sp) =>
                           {
                               var userManager = sp.GetRequiredService<UserManager<TUser>>();
        ...
        routeGroup.MapPost("/forgotPassword",
                           async Task<Results<Ok, ValidationProblem>> ([FromBody] ForgotPasswordRequest resetRequest,
                                                                      [FromServices] IServiceProvider sp) =>
                           {
                               var userManager = sp.GetRequiredService<UserManager<TUser>>();
        ...
        routeGroup.MapPost(
            "/resetPassword",
            async Task<Results<Ok, ValidationProblem>> ([FromBody] ResetPasswordRequest resetRequest,
                                                       [FromServices] IServiceProvider sp) =>
            {
                var userManager = sp.GetRequiredService<UserManager<TUser>>();
                ...
                {
                    // Don't reveal that the user does not exist or is not confirmed, so don't return a 200 if we would
                    // have returned a 400 for an invalid code given a valid user email.
            ...
            "/info", async Task<Results<Ok<InfoResponse>, ValidationProblem, NotFound>> (
                         ...
                         [FromServices] IServiceProvider sp) =>
                     {
                         ...
                         if (!string.IsNullOrEmpty(infoRequest.NewPassword))
                         {
                             ...
                             {
                                 return CreateValidationProblem(
                                     "OldPasswordRequired", "The old password is required to set a new password. If " +
                                                                "the old password is forgotten, use /resetPassword.");
                             }
                                              ...
                                              string email, bool isChange = false)
        {
            ...
            var code = isChange ? await userManager.GenerateChangeEmailTokenAsync(user, email)
