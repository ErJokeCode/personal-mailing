...
def get_sections_keyboard(data_course: OnboardCourse, from_add_course: bool = False, crossed_topics:dict = None) -> InlineKeyboardMarkup:
    ...
    if from_add_course:
        list_btn.append([InlineKeyboardButton(text="Назад", callback_data="additional_courses")])
    else:
        list_btn.append([InlineKeyboardButton(text="Главное меню", callback_data="end_onboarding")])
...
def get_topics_keyboard(data_course: dict, callback_data_section: str, crossed_topics:dict = None) -> InlineKeyboardMarkup:
    ...
    list_btn.append([InlineKeyboardButton(text="К разделам", callback_data="onboarding_sections")])
...
def topic_keyboard(data_course: dict, callback_data_topic: str, is_help=False, not_question=False):
    ...
    if not_question == False:
        list_btn2 = []
        if is_help == False:
            list_btn2.append(InlineKeyboardButton(text="Подсказка", callback_data=f"{callback_data_topic}____help"))
        list_btn2.append(InlineKeyboardButton(text="Чат с куратором", url=URL_BOT_CHAT_CURATOR))
