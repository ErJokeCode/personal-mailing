...
async def all_message(message: types.Message, state: FSMContext):
    ...
    if user_id == None:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(f"{URL_CORE}/core/admins", headers=headers) as response_admins:
                if response_admins.status == 200:
                    with aiohttp.MultipartWriter('form-data') as mpwriter:
                        ...
                        async with session.post(f"{URL_CORE}/core/chats/from-student", headers=headers, data=mpwriter) as resp:
                            if resp.status == 200:
