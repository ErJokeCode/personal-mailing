...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    if not user_data.get('email') and not user_data.get('personal_number'):
        welcome_msg = await message.answer("Добро пожаловать! Пожалуйста, введите вашу почту.")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{URL_SERVER}/user/auth", json=body) as response:
                if response.status == 200:
                    ...
                    await message.bot.delete_messages(message.chat.id, [msg_login.message_id, message.message_id])
                else:
                    await message.answer("Произошла ошибка при регистрации. Пожалуйста, введите вашу почту.")
                    await state.set_state(RegistrationStates.WAITING_FOR_EMAIL)
                    await state.update_data(welcome_msg_id=welcome_msg.message_id)
...
async def process_email(message: types.Message, state: FSMContext):
    if is_valid_email(message.text):
        ...
        student_id_msg = await message.answer("Спасибо! Теперь введите номер вашего студенческого билета.")
...
async def process_student_id(message: types.Message, state: FSMContext):
    await state.update_data(personal_number=message.text, student_id_response_msg_id=message.message_id)
    user_data = await state.get_data()
    body = {
            "email": user_data.get('email'),
            "personal_number": user_data.get('personal_number'),
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(f"{URL_SERVER}/user/auth", json=body) as response:
            if response.status == 200:
                ...
                for msg in ['welcome_msg_id', 'email_msg_id', 'student_id_msg_id', 'student_id_response_msg_id']:
                    msg_id = user_data.get(msg)
                    if msg_id:
                        await message.bot.delete_message(chat_id=message.chat.id, message_id=msg_id)
                await state.set_state(LKStates.MAIN_MENU)
                await state.clear()
                await state.update_data(email = body['email'], personal_number = body["personal_number"], user_id=response_data.get('_id'))
                await show_main_menu(message, state)
            else:
                await message.answer("Произошла ошибка при регистрации. Пожалуйста, введите вашу почту.")
                await state.set_state(RegistrationStates.WAITING_FOR_EMAIL)
