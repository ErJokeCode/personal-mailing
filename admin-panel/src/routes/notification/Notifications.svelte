<script lang="ts">
    ...
    onMount(async () => {
        ...
        try {
            response = await fetch(`${server_url}/core/notifications`, {
                credentials: "include",
            });
        } catch (err) {}
    ...
    $: filtered = notifications.filter(
        (item) =>
            item.content.toLowerCase().indexOf(searchTerm.toLowerCase()) !==
                -1 ||
            item.createdAt.toLowerCase().indexOf(searchTerm.toLowerCase()) !==
                -1 ||
    ...
    <div class="relative h-full w-full overflow-y-auto lg:ml-64 pt-[70px]">
        ...
        >
            <div class="pt-4 px-6">
                ...
                <Button
                    on:click={() => navigate("/notifications/send")}
                    class="mb-3"
                >
                    Создать рассылки
                ...
                <Search
                    size="md"
                    class="block w-80 p-2.5 ps-10 text-sm mb-4"
                    placeholder="Поиск"
            ...
            <Table hoverable={true}>
                ...
                <TableBody tableBodyClass="divide-y">
                    {#each filtered as notification}
                        ...
                        >
                            <TableBodyCell class="px-8 break-all"
                                >{notification.content.length > 50
                                    ? notification.content.slice(0, 50)
                                    : notification.content}</TableBodyCell
                            >
                            <TableBodyCell class="px-8"
                                >{notification.date}</TableBodyCell
                            >
                            <TableBodyCell
                                class="px-8 overflow-hidden text-ellipsis"
                                style="max-width: 50dvw"
                            >
