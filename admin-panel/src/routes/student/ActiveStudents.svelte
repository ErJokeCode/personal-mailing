<script lang="ts">
    ...
    onMount(async () => {
        ...
        activeStudents =
            (await http.get("/core/students", studentStatus)).items ?? [];
        studentStatus = studentStatus.end_load();

        signal.on("StudentAuthed", handleNewStudent);
    });

    onDestroy(async () => {
        signal.off("StudentAuthed", handleNewStudent);
    });
        ...
        >
            ...
            <Table hoverable={true}>
                <TableHead>
                    ...
                    <TableHeadCell class="px-8"
                        >Номер студенческого</TableHeadCell
                    >
                </TableHead>
                <TableBody>
                        ...
                        {#if student.info !== null}
                            <TableBodyRow on:click={() => fullInfo(student.id)}>
                                <TableBodyCell class="px-8"
                                    >{student.info.email}</TableBodyCell
                                >
                                <TableBodyCell class="px-8"
                                    >{student.info.group
                                        .numberCourse}</TableBodyCell
                                >
                                <TableBodyCell class="px-8"
                                    >{student.info.group.number}</TableBodyCell
                                >
                                <TableBodyCell class="px-8"
                                    >{student.info.surname}</TableBodyCell
                                >
                                <TableBodyCell class="px-8"
                                    >{student.info.name}</TableBodyCell
                                >
                                <TableBodyCell class="px-8"
                                    >{student.info.patronymic}</TableBodyCell
                                >
                                <TableBodyCell class="px-8"
                                    >{student.info.typeOfCost}</TableBodyCell
                                >
                                <TableBodyCell class="px-8"
                                    >{student.info
                                        .personalNumber}</TableBodyCell
                                >
                            </TableBodyRow>
                        {/if}
                    {/each}
